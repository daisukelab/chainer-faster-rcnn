    ...
    for cls_id in range(1, 21):
        ...
        for i in inds:
            ...
            ret, baseline = cv.getTextSize(
                CLASSES[cls_id], cv.FONT_HERSHEY_SIMPLEX, 0.6, 1)
            cv.rectangle(out, (x1, y2 - ret[1] - baseline),
                         (x1 + ret[0], y2), (0, 0, 255), -1)
            cv.putText(out, CLASSES[cls_id], (x1, y2 - baseline),
                       cv.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1, CV_AA)

    return out

def process_one(orig_image, model):
...
if __name__ == '__main__':
    ...
    parser.add_argument('input', default='0',
                        help='Camera # or path for the pictures')
    ...
    # initialize source
    vc = MyVideoCapture(args.input)
    # loop
    paceKeeper = PaceKeeper(1)
    timer = CvTimer()
    rval = True
    key  = 0
    while rval:
        rval, cur = vc.read()
        if not rval:
            print('video input failure')
            break
        result = process_one(cur, model)
